...
import { wellKnownProvider, type WellKnownSkill } from './providers/wellknown.ts';
...
async function writeMapFiles(targetDir: string, files: Map<string, string>): Promise<void> {
  for (const [path, contents] of files) {
    await writeSafeFile(targetDir, path, contents);
  }
}

async function writeSafeFile(targetDir: string, filePath: string, contents: string): Promise<void> {
